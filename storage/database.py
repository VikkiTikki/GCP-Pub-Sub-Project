...
class Database:
    ...
    def create_second_subscriber_table(self):
        if not self.conn or not self.cursor:
            print("No database connection. Second subscriber table not created.")
            return

        query = """
        CREATE TABLE IF NOT EXISTS sub2_received_messages (
            id INT AUTO_INCREMENT PRIMARY KEY,
            message_id VARCHAR(255),
            content TEXT,
            publish_time DATETIME,
            receive_time DATETIME,
            latency_ms INT,
            INDEX idx_second_receive_time (receive_time)
        );
        """
        self.cursor.execute(query)
        self.conn.commit()

    def insert_second_received_message(
        self,
        message_id,
        content,
        publish_time,
        ...
        if not self.conn or not self.cursor:
            print("No database connection. Cannot insert second subscriber message.")
            return False

        latency = int((receive_time - publish_time).total_seconds() * 1000)

        query = """
        INSERT INTO sub2_received_messages
        (message_id, content, publish_time, receive_time, latency_ms)
        VALUES (%s, %s, %s, %s, %s)
        """
        values = (
            message_id,
        ...
        try:
            ...
            print(f"MySQL insert error (second subscriber): {e}")
            return False
